...
func (api *TransactionAPI) PendingTransactionsFrom(addr common.Address) ([]*RPCTransaction, error) {
...
func (api *BundleAPI) CallBundle(ctx context.Context, args CallBundleArgs) (map[string]interface{}, error) {
	if len(args.Txs) == 0 {
	...
	state, parent, err := api.b.StateAndHeaderByNumberOrHash(ctx, args.StateBlockNumberOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	...
	} else if api.b.ChainConfig().IsLondon(big.NewInt(args.BlockNumber.Int64())) {
		baseFee = eip1559.CalcBaseFee(api.b.ChainConfig(), parent)
	}
	header := &types.Header{
	...
	for i, tx := range txs {
		...
		receipt, result, err := core.ApplyTransactionWithResult(api.b.ChainConfig(), api.chain, &coinbase, gp, state, header, tx, &header.GasUsed, vmconfig)
		if err != nil {
